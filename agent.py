...
def node_nudge(state):
    ...
    막으려는 것은 **근거 없이 지어낸 답**이지 넘기기가 아니다.
    처음에는 "넘기지 마라" 라고 적었다가, 개별 조직의 결함 여부를 확답해 달라는 문의까지
    억지로 검색시켜 답하게 만들었다. 두 길을 다 열어 두고 고르게 해야 한다.
    """
    return {"nudged": True, "messages": [HumanMessage(
        "아직 문서를 하나도 열지 않았다. 네가 아는 지식만으로 답하지 마라.\n"
        "둘 중 하나를 골라라.\n"
        "  (1) 문서로 답할 수 있는 문의다 → 위 목차에서 문의에 가장 가까운 조항을 골라, "
        "그 조항 제목에 나온 낱말을 검색어로 삼아 도구를 한 번 불러라.\n"
        "  (2) 문서로 답할 수 없는 문의다(개별 조직의 심사 결과 확답, 범위 밖 주제 등) "
        "→ escalate_to_expert 를 불러라.\n"
        "둘 다 하지 않고 그냥 글로 답하지는 마라.")]}
