...
| 반복 횟수 | 3회 | 5회 | 3회 |
| 회차 간 폭 | ±1%p / ±3%p | 0%p / ±3%p | 0%p / ±2.5%p |

**카테고리 분류** — 답변용 평가셋과 별도로, 경계를 찌르는 문항 32건을 따로 모아 쟀다
(`data/routing_set.json` · 3회 = 96개 판정).
...
혼동 행렬과 오분류 4건의 원인 분석은 [분류 성능](#분류-성능--정확도--macro-f1--혼동-행렬) 에 있다.
분류 규칙을 손볼 때 보지 않은 `goldenset` 20건으로 다시 재면 정확도·macro F1 모두 1.000 이다.

**② 가 83.8% → 66.7% 로 떨어지는 것이 이 프로젝트의 가장 중요한 결과다.**
보류해 둔 4건 중 3건이 실패했고, 그중 둘은 이미 알고 있던 실패 유형의 재발이었다.
자세한 내용은 [과적합을 확인했다](#과적합을-확인했다) 에 적었다.
카테고리 분류는 보류 문항에서도 100% 라, 라우팅 설계는 일반화됐다고 볼 수 있다.
...
4. **단정 표현** — 심사원 판단이 필요한 사안을 확정으로 말한 경우
...
그래서 화면을 파이프라인 순서대로 늘어놓고 네 가지를 답과 나란히 놓는다.

| 화면 요소 | 보여 주는 것 |
|---|---|
| **① 판정된 카테고리 · 확신도 · 판정 근거** | 왜 이 문서를 골랐는지 |
| **답변** | 본문. 넘긴 경우 이관했다고 먼저 알린다 |
| **호출한 도구** | `search_notice` → 인증 고시 처럼 사람이 읽는 이름을 같이 |
| **④ 검증 결과** | 통과 / 지적 사항. 근거에 없는 숫자·조항, 단정 표현을 잡는다 |
| **근거로 읽은 문서 조항** | 조각 id + 섹션 경로를 접이식으로. 펼치면 원문 전문과 출처 URL |

왼쪽에는 근거 문서 구성(410개 조각)과 다섯 카테고리를 늘 띄워 둔다.
예시 문의 10개는 평가셋에서 성격이 다른 것들을 골랐다 — 조항 인용형, 수치형,
경계 사례(ISO 27001), 넘기기 두 갈래가 모두 들어 있다.

### 화면 캡처

**답변 경로** — "인증서 유효기간이 얼마나 되나요? 인증을 계속 유지하려면 무엇을 해야 하나요?"

![데모 화면 — 답변 경로](docs/screenshot-main.png)

`CERT` 로 판정(확신도 0.90)하고 `search_notice` 하나만 불러 고시 4개 조각을 읽었다.
답변은 유효기간 3년(제32조) · 만료 3개월 전 갱신(제28조) · 미신청 시 효력 상실(제28조③)을
모두 담았다. **그런데 ④ 검증이 걸었다** — "항을 잘못 짚음: 제32조제1항 에 3 은 없다".
3년은 제32조**제3항**이다. 검증 단계가 실제로 오류를 잡는 장면이라 이 캡처를 그대로 두었다.

**넘기기 경로** — "저희 회사는 방화벽 정책을 반기에 한 번만 검토하는데, 2.6.1 결함이 안 나올 거라고 확답해 주실 수 있나요?"

![데모 화면 — 넘기기 경로](docs/screenshot-detail.png)

카테고리는 `PROTECT` 로 **옳게** 판정했다(2.6 접근통제 문의이므로). 기준을 설명할 수는 있다.
그런데 개별 조직의 심사 결과를 미리 판정해 달라는 요구이므로 모델이 `escalate_to_expert` 를
불렀고, 검색 도구는 하나도 부르지 않아 **읽은 근거가 0개**다.
"분류는 맞지만 답하지 않는다" 가 되는 자리이고, 평가셋 G-20 이 재는 것이 바로 이 동작이다.
...
python evaluate.py -n 5         # 두 지표 측정 (5회 평균)
python evaluate.py -n 3 --split all   # 보류 4건까지 포함해 재기
...
data/goldenset.json     답변 평가셋 20건 + 교정 기록
data/routing_set.json   분류 평가셋 32건 (경계 문항 포함)
